...
#include "se2_parameterization.h"
...
bool LikelihoodField::AlignCeres(SE2& init_pose) {
    ...
    options.linear_solver_type         = ceres::DENSE_QR;  // DENSE_QR DENSE_SCHUR SPARSE_SCHUR
    options.num_threads                = 4;

    // 参数块: 添加待优化变量
    double pose2d[3] = {0};
    pose2d[0] = init_pose.translation()[0];
    pose2d[1] = init_pose.translation()[1];
    pose2d[2] = init_pose.so2().log();
    ceres::LocalParameterization *parameterization = new (SE2Parameterization);
    problem.AddParameterBlock(pose2d, 3, parameterization);
    ...
    for (size_t i = 0; i < source_->ranges.size(); ++i) {
        ...
        auto factor = new LikelihoodFactor(field_, r, angle);
        problem.AddResidualBlock(factor, loss_function, pose2d);
    }

    options.max_num_iterations = 100;
    solver.Solve(options, &problem, &summary);

    init_pose.translation() = Eigen::Vector2d(pose2d[0], pose2d[1]);       // x, y    
