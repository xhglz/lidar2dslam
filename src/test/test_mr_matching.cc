...
int main(int argc, char** argv) {
    ...
    std::ifstream fin("./data/output/loops.txt");
    int loop_id = 0;
    ...
    while (!fin.eof()) {
        ...
        cv::Mat occu_map = cv::imread("./data/output/submap_" + std::to_string(submap_id) + ".png", cv::IMREAD_GRAYSCALE);
        cv::Mat occu_map_color =
            cv::imread("./data/output/submap_" + std::to_string(submap_id) + ".png", cv::IMREAD_COLOR);
        mr_field.SetFieldImageFromOccuMap(occu_map);

        sad::Frame frame;
        frame.Load("./data/output/frame_" + std::to_string(frame_id) + ".txt");
